...
readonly class StandardSecurity {
    /** @see 7.6.4.3.2 a */
    ...
    public function getFileEncryptionKey(EncryptDictionary $encryptDictionary, string $firstID): ?FileEncryptionKey {
        if ($this->ownerPassword !== null) {
            ...
            if ($this->isFileEncryptionKeyValid($fileEncryptionKey, $encryptDictionary, $firstID)) {
                return $fileEncryptionKey;
            }
        }

        $fileEncryptionKey = $this->getFileEncryptionKeyForUserPassword($this->getPaddedUserPassword(), $encryptDictionary, $firstID);
        if ($this->isFileEncryptionKeyValid($fileEncryptionKey, $encryptDictionary, $firstID)) {
            return $fileEncryptionKey;
        }
    ...
    /** @see 7.6.4.4.6 */
    private function recoverPaddedUserPasswordFromOwnerEntry(EncryptDictionary $encryptDictionary): string {
        ...
        if ($encryptDictionary->getStandardSecurityHandlerRevision() === StandardSecurityHandlerRevision::v2) {
            $ownerPasswordDecryptionKey = substr($md5Hash, 0, 5);
        } else {
            $ownerPasswordDecryptionKey = substr($md5Hash, 0, $fileEncryptionKeyLengthInBytes);
        }

        $ownerPasswordEntry = $encryptDictionary->getOwnerPasswordEntry();
        if ($encryptDictionary->getStandardSecurityHandlerRevision() === StandardSecurityHandlerRevision::v2) {
            $paddedUserPassword = RC4::crypt($ownerPasswordDecryptionKey, $ownerPasswordEntry);
        } else {
            $paddedUserPassword = $ownerPasswordEntry;
            for ($i = 19; $i >= 0; $i--) {
                $paddedUserPassword = RC4::crypt(
                    implode('', array_map(
                        fn($c) => chr(ord($c) ^ $i),
                        str_split($ownerPasswordDecryptionKey),
                    )),
                    $paddedUserPassword,
                );
            }
    ...
    private function getPaddedUserPassword(): string {
    ...
    /** @see 7.6.4.3.2 step a */
    private function getPaddedOwnerPassword(): string {
        return substr($this->ownerPassword ?? '', 0, self::PASSWORD_LENGTH)
            . substr(self::PADDING_STRING, 0, max(0, self::PASSWORD_LENGTH - strlen($this->ownerPassword ?? '')));
    }
}
